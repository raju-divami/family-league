...
public class MatchPredictionController {
    ...
    public ResponseEntity<ApiResponse<MatchPredictionResponse>> submit(
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.ok("Prediction submitted",
                predictionService.submitPrediction(matchId, principal.getId(), request)));
    }

    @PutMapping
    @Operation(summary = "Update match prediction")
    public ResponseEntity<ApiResponse<MatchPredictionResponse>> update(
            @PathVariable Long matchId,
            @Valid @RequestBody SubmitMatchPredictionRequest request,
            @AuthenticationPrincipal UserPrincipal principal) {
        return ResponseEntity.ok(ApiResponse.ok("Prediction updated",
    ...
    public ResponseEntity<ApiResponse<MatchPredictionResponse>> getMyPrediction(
            @PathVariable Long matchId,
            @AuthenticationPrincipal UserPrincipal principal) {
        return ResponseEntity.ok(ApiResponse.ok(predictionService.getMyPrediction(matchId, principal.getId())));
    }

    @GetMapping
    @Operation(summary = "Get all predictions for a match (visible after lock)")
    public ResponseEntity<ApiResponse<List<MatchPredictionResponse>>> getAllPredictions(
            @PathVariable Long matchId,
            @AuthenticationPrincipal UserPrincipal principal) {
